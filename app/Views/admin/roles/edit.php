        ...
        <div class="card-body">
            <form action="<?= base_url('admin/roles/update/' . $role['id']) ?>" method="post">
                <div class="mb-3">
                    ...
                    <input type="text" name="name" id="name" class="form-control" value="<?= old('name', $role['name']) ?>" required pattern="[a-z_]+" <?= $isCore ? 'readonly' : '' ?>>
                    <?php if ($isCore): ?>
                        <small class="text-danger">Core roles cannot have their slug changed.</small>
                    <?php endif; ?>
                </div>
                <div class="mb-3">
